...
async function fetchMyClasses() {
  try {
    const res = await fetch('https://localhost:3700/api/student/classes/my', {
...
async function fetchAvailableClasses() {
  try {
    const res = await fetch('https://localhost:3700/api/student/classes', {
      headers: { 'Authorization': 'Bearer ' + token }
    });
    if (!res.ok) {
      const data = await res.json().catch(() => ({}));
...
async function enroll(classId, className, teacherName) {
  try {
    const res = await fetch('https://localhost:3700/api/student/classes/enroll', {
      method: 'POST',
...
async function unenroll(classId, btn) {
  try {
    const res = await fetch('https://localhost:3700/api/student/classes/unenroll', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + token },
      body: JSON.stringify({ class_id: classId })
    });
